...
export const fetchPosts = createAsyncThunk(
  ...
  async (params) => {
    ...
    const { data } = await $api.get(`/posts/${sort}/${asc}/${page}/9`);
    return data;
  }
);
...
export const fetchEditPost = createAsyncThunk(
  "posts/fetchEditPost",
  async ({ id, text, title, price1, price2, price3, imageUrl }) => {
    const { data } = await $api.patch(`/posts/edit/${id}`, {
      imageUrl,
      title,
      text,
      price1,
    ...
    return { id, text, title, price1, price2, price3, imageUrl };
  }
);
...
const postSlice = createSlice({
  ...
  extraReducers: {
    ...
    [fetchPostTitles.fulfilled]: (state, action) => {
      ...
      state.pageCount = Math.ceil(action.payload.length / 9);
    ...
    [fetchCreatePost.fulfilled]: (state, action) => {
      state.post.posts = state.post.posts.filter(
      ...
      state.post.posts = state.post.posts.slice(0, 9);
    },
